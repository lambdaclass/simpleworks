...
mod test {
    ...
    fn end_to_end() {
        ...
        assert!(!bad_tx.validate(&pp, &state, &mut rng).unwrap());
        assert!(matches!(
            state.apply_transaction(&pp, &bad_tx, &mut rng),
            None
        ));
