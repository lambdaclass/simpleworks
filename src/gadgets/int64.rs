...
/// unsigned integer.
#[derive(Clone, Debug)]
pub struct Int64<F: Field> {
    /// Little-endian representation: least significant bit first
    pub(crate) bits: [Boolean<F>; I64_SIZE_IN_BITS],
    pub(crate) value: Option<i64>,
}

impl<F: Field> Int64<F> {
    /// Construct a constant `Int64` from a `i64`
    ///
    ...
    /// use simpleworks::gadgets::int64::Int64;
    /// use ark_bls12_381::Fr;
    /// use ark_relations::r1cs::*;
    /// use ark_r1cs_std::prelude::*;
    ///
    /// let cs = ConstraintSystem::<Fr>::new_ref();
    /// let var = Int64::new_witness(cs.clone(), || Ok(2))?;
    ///
    /// let constant = Int64::constant(2);
    ...
    pub fn constant(value: i64) -> Self {
        let mut bits = [Boolean::FALSE; I64_SIZE_IN_BITS];
        ...
        F: PrimeField,
    {
        ...
        // the result
        let mut max_value = BigInt::from(i64::max_value()) * BigInt::from(OPERANDS_LEN);
        ...
        result_bits.truncate(I64_SIZE_IN_BITS);
        ...
        match modular_value {
            ...
            Some(Err(e)) => bail!("{e}"),
            None => bail!("The result of the modular addition between Int64 is None"),
        }
    }
}

impl<ConstraintF: Field> AllocVar<i64, ConstraintF> for Int64<ConstraintF> {
    fn new_variable<T: Borrow<i64>>(
...
impl<F: Field> ToBitsGadget<F> for Int64<F> {
    fn to_bits_le(&self) -> Result<Vec<Boolean<F>>, SynthesisError> {
        Ok(self.bits.to_vec())
    }
}

impl<F: Field> R1CSVar<F> for Int64<F> {
    type Value = i64;
...
mod tests {
    ...
    fn test_int8_from_bits_to_bits() {
        ...
        let byte =
            Int64::new_witness(ark_relations::ns!(cs, "alloc value"), || Ok(byte_val)).unwrap();
