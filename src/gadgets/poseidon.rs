...
pub(crate) fn poseidon_parameters_for_test<F: PrimeField>() -> Result<PoseidonParameters<F>> {
    let alpha = 17;
    let mds = vec![
        vec![
            F::from_str(
            ...
            .map_err(|_e| anyhow!("Error generating field element from str"))?,
            F::from_str(
...
mod tests {
    ...
    fn test_poseidon2_hash_primitive_and_gadget_implementations_comparison() {
        ...
        let primitive_squeeze = native_sponge.squeeze_bytes(input.iter().flatten().count());

        let input_var: Vec<_> = input
            .iter()
            .map(|v| UInt8Gadget::new_input_vec(ns!(cs, "input"), v).unwrap())
            .collect();
