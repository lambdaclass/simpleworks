...
impl<F: Field> Int16<F> {
    ...
    /// use simpleworks::gadgets::int16::Int16;
    /// use ark_bls12_381::Fr;
    /// use ark_relations::r1cs::*;
    /// use ark_r1cs_std::prelude::*;
    ///
    /// let cs = ConstraintSystem::<Fr>::new_ref();
    /// let var = Int16::new_witness(cs.clone(), || Ok(2))?;
    ///
    /// let constant = Int16::constant(2);
    ...
    pub fn constant(value: i16) -> Self {
        let mut bits = [Boolean::FALSE; I16_SIZE_IN_BITS];
        ...
        F: PrimeField,
    {
        ...
        // the result
        let mut max_value = BigInt::from(i16::max_value()) * BigInt::from(OPERANDS_LEN);
        ...
        let modular_value = result_value.clone().map(|v| {
            let modulus = BigInt::from(1_u64)
                << (I16_SIZE_IN_BITS
                    .to_u32()
                    .ok_or("I16_SIZE_IN_BITS value cannot be represented as u32.")?);
            (v % modulus)
                .to_i16()
                .ok_or("Modular value cannot be represented as i16.")
        });
        ...
        result_bits.truncate(I16_SIZE_IN_BITS);
        ...
        match modular_value {
            ...
            Some(Err(e)) => bail!("{e}"),
            None => bail!("The result of the modular addition between Int16 is None"),
        }
    }
}

impl<ConstraintF: Field> AllocVar<i16, ConstraintF> for Int16<ConstraintF> {
    fn new_variable<T: Borrow<i16>>(
        ...
        let mut values = [None; I16_SIZE_IN_BITS];
...
impl<ConstraintF: Field> EqGadget<ConstraintF> for Int16<ConstraintF> {
...
impl<F: Field> ToBitsGadget<F> for Int16<F> {
    fn to_bits_le(&self) -> Result<Vec<Boolean<F>>, SynthesisError> {
        Ok(self.bits.to_vec())
    }
}

impl<F: Field> R1CSVar<F> for Int16<F> {
    type Value = i16;
...
mod tests {
    ...
    #[test]
    fn test_int16_from_bits_to_bits() {
        let cs = ConstraintSystem::<Fr>::new_ref();
        let byte_val = 0b01110001;
        let byte =
            Int16::new_witness(ark_relations::ns!(cs, "alloc value"), || Ok(byte_val)).unwrap();
    ...
    fn test_addition_with_positive_operands() {
        ...
        let result_from_primitive_var = Int16::new_witness(ark_relations::ns!(cs, "result"), || Ok(primitive_result)).unwrap();
        let addend_var = Int16::new_witness(ark_relations::ns!(cs, "addend"), || Ok(primitive_addend)).unwrap();
        let augend_var = Int16::new_witness(ark_relations::ns!(cs, "augend"), || Ok(primitive_augend)).unwrap();
        
        let result = Int16::addmany(&[addend_var, augend_var]).unwrap();
    ...
    fn test_addition_with_overflow() {
        ...
        let primitive_addend = i16::max_value();
        let primitive_augend = 1;
        
        let addend_var = Int16::new_witness(ark_relations::ns!(cs, "addend"), || Ok(primitive_addend)).unwrap();
        let augend_var = Int16::new_witness(ark_relations::ns!(cs, "augend"), || Ok(primitive_augend)).unwrap();
        
        let result = Int16::addmany(&[addend_var, augend_var]);
